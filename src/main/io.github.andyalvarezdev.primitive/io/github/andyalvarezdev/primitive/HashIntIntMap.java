...
import io.github.andyalvarezdev.primitive.function.*;
...
import static io.github.andyalvarezdev.primitive.Constants.DEFAULT_INT_VALUE;
 ...
 * @see     Hashtable
 */
public class HashIntIntMap extends AbstractIntIntMap
     ...
     * Comparable. (If neither of these applyAsInt, we may waste about a
     ...
     */
    final void putMapEntries(IntIntMap m, boolean evict) {
        int s = m.size();
        if (s > 0) {
    ...
    public int get(int key) {
        Node e;
        return (e = getNode(hash(key), key)) == null ? DEFAULT_INT_VALUE : e.value;
    }
    ...
    final Node getNode(int hash, int key) {
        Node[] tab; Node first, e; int n; int k;
     ...
     */
    final int putVal(int hash, int key, int value, boolean onlyIfAbsent,
                   boolean evict) {
        ...
        if ((p = tab[i = (n - 1) & hash]) == null)
            ...
            if (e != null) { // existing mapping for key
                ...
                if (!onlyIfAbsent || oldValue == DEFAULT_INT_VALUE)
                    e.value = value;
                afterNodeAccess(e);
                return oldValue;
            }
    ...
    public int remove(int key) {
        Node e;
        return (e = removeNode(hash(key), key, DEFAULT_INT_VALUE, false, true)) == null ?
                DEFAULT_INT_VALUE : e.value;
    }
    ...
    final Node removeNode(int hash, int key, int value,
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            ...
            if (node != null && (!matchValue || node.value == value)) {
    ...
    final class KeySet extends AbstractIntSet {
        ...
        public final void clear()               { HashIntIntMap.this.clear(); }
        public final PrimitiveIterator.OfInt iterator()     { return new KeyIterator(); }
        ...
        public final Spliterator.OfInt spliterator() {
            return new KeySpliterator(HashIntIntMap.this, 0, -1, 0, 0);
        }
    ...
    final class Values extends AbstractIntCollection {
        public final int size()                 { return size; }
        public final void clear()               { HashIntIntMap.this.clear(); }
        public final PrimitiveIterator.OfInt iterator()     { return new ValueIterator(); }
        public final boolean contains(int o) { return containsValue(o); }
        public final Spliterator.OfInt spliterator() {
            return new ValueSpliterator(HashIntIntMap.this, 0, -1, 0, 0);
        }
        public final void forEach(IntConsumer action) {
            Node[] tab;
            if (action == null)
    ...
    final class EntrySet extends AbstractSet<IntIntMap.Entry> {
        public final int size()                 { return size; }
        public final void clear()               { HashIntIntMap.this.clear(); }
        ...
        public final Spliterator<IntIntMap.Entry> spliterator() {
            return new EntrySpliterator(HashIntIntMap.this, 0, -1, 0, 0);
        }
    ...
    public boolean replace(int key, int oldValue, int newValue) {
        Node e;
        if ((e = getNode(hash(key), key)) != null && e.value == oldValue) {
            e.value = newValue;
            afterNodeAccess(e);
            return true;
        }
    ...
    public int computeIfAbsent(int key,
                             ToIntIntFunction mappingFunction) {
        ...
        if ((first = tab[i = (n - 1) & hash]) != null) {
            ...
            int oldValue;
            if (old != null && (oldValue = old.value) != DEFAULT_INT_VALUE) {
                afterNodeAccess(old);
                return oldValue;
            }
        }
     ...
     */
    @Override
    public int computeIfPresent(int key,
        ...
        if ((e = getNode(hash, key)) != null &&
                (oldValue = e.value) != DEFAULT_INT_VALUE) {
            int mc = modCount;
            int v = remappingFunction.applyAsInt(key, oldValue);
            if (mc != modCount) { throw new ConcurrentModificationException(); }
            if (v != DEFAULT_INT_VALUE) {
                e.value = v;
                afterNodeAccess(e);
                ...
                removeNode(hash, key, DEFAULT_INT_VALUE, false, true);
        }
        return DEFAULT_INT_VALUE;
    }
     ...
     */
    @Override
    public int compute(int key, IntToIntBiFunction remappingFunction) {
        if (remappingFunction == null)
            throw new NullPointerException();
        int hash = hash(key);
        Node[] tab; Node first; int n, i;
        ...
        int oldValue = (old == null) ? DEFAULT_INT_VALUE : old.value;
        int mc = modCount;
        int v = remappingFunction.applyAsInt(key, oldValue);
        if (mc != modCount) { throw new ConcurrentModificationException(); }
        if (old != null) {
    ...
    public int merge(int key, int value, IntToIntBiFunction remappingFunction) {
        ...
        if (old != null) {
            int v;
            if (old.value != DEFAULT_INT_VALUE) {
                int mc = modCount;
                v = remappingFunction.applyAsInt(old.value, value);
                if (mc != modCount) {
                    throw new ConcurrentModificationException();
                }
            } else {
    ...
    private void readObject(java.io.ObjectInputStream s)
        ...
        if (mappings < 0)
            ...
            SharedSecrets.getJavaObjectInputStreamAccess().checkArray(s, IntIntMap.Entry[].class, cap);
            table = new Node[cap];
    ...
    final class KeyIterator extends HashIterator
        ...
        public final int nextInt() { return nextNode().key; }
    }
    ...
    static class HashMapSpliterator {
        ...
        HashMapSpliterator(HashIntIntMap m, int origin,
                           int fence, int est,
    ...
    static final class KeySpliterator
        ...
        KeySpliterator(HashIntIntMap m, int origin, int fence, int est, int expectedModCount) {
    ...
    static final class ValueSpliterator
            extends HashMapSpliterator
            implements Spliterator.OfInt {
        ValueSpliterator(HashIntIntMap m, int origin, int fence, int est,
                         int expectedModCount) {
            super(m, origin, fence, est, expectedModCount);
        }
    ...
    static final class EntrySpliterator
            extends HashMapSpliterator
            implements Spliterator<IntIntMap.Entry> {
        EntrySpliterator(HashIntIntMap m, int origin, int fence, int est,
                         int expectedModCount) {
            super(m, origin, fence, est, expectedModCount);
        }
        ...
        public void forEachRemaining(Consumer<? super IntIntMap.Entry> action) {
            int i, hi, mc;
            if (action == null)
                throw new NullPointerException();
            HashIntIntMap m = map;
        ...
        public boolean tryAdvance(Consumer<? super IntIntMap.Entry> action) {
            int hi;
            if (action == null)
                throw new NullPointerException();
            Node[] tab = map.table;
     ...
     * linked node.
     */
    static final class TreeNode extends LinkedHashIntIntMap.Entry {
         ...
         */
        static <K,V> void moveRootToFront(Node[] tab, TreeNode root) {
            int n;
         ...
         * comparing keys.
         */
        final TreeNode find(int h, int k, Class<?> kc) {
            TreeNode p = this;
            do {
                int ph, dir, pk;
         ...
         */
        final TreeNode getTreeNode(int h, int k) {
         ...
         * this node.
         */
        final Node untreeify(HashIntIntMap map) {
         ...
         */
        final TreeNode putTreeVal(HashIntIntMap map, Node[] tab,
                                     int h, int k, int v) {
            Class<?> kc = null;
         ...
         */
        final void removeTreeNode(HashIntIntMap map, Node[] tab,
                                  boolean movable) {
            int n;
         ...
         */
        final void split(HashIntIntMap map, Node[] tab, int index, int bit) {
            TreeNode b = this;
