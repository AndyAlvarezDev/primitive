 ...
 * have no effect on the map.  For example, invoking the {@link #putAll(IntDoubleMap)}
 ...
 * @see IntSet
 */
public interface IntDoubleMap extends Container
{
	// Query Operations
	...
	default double computeIfAbsent(int key, IntToDoubleFunction mappingFunction) {
		...
		double v;
		if(!containsKey(key)) {
			v =  mappingFunction.applyAsDouble(key);
			put(key, v);
			return v;
		}
		return get(key);
	 ...
	 * of calling {@link #put(int, double) put(k, v)} on this map once
	...
	/**
	 * Returns a {@link DoubleCollection} view of the values contained in this map.
