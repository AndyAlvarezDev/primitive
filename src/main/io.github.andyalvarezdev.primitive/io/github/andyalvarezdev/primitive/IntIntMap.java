...
import io.github.andyalvarezdev.primitive.function.IntIntBiConsumer;
import io.github.andyalvarezdev.primitive.function.IntToIntBiFunction;
import io.github.andyalvarezdev.primitive.function.ToIntIntFunction;
 ...
 * have no effect on the map.  For example, invoking the {@link #putAll(IntIntMap)}
...
public interface IntIntMap {
    ...
    interface Entry {
         ...
         * @since 1.8
         */
        public static Comparator<Entry> comparingByKey() {
            return (Comparator<Entry> & Serializable)
                    (c1, c2) -> Integer.compare(c1.getKey(), c2.getKey());
        }
         ...
         * @since 2.0
         */
        public static  Comparator<Entry> comparingByValue() {
            return (Comparator<Entry> & Serializable)
                    (c1, c2) -> Integer.compare(c1.getValue(), c2.getValue());
        }
         ...
         * @since 2.0
         */
        public static Comparator<Entry> comparingByKey(IntComparator cmp) {
         ...
         * @since 1.8
         */
        public static Comparator<Entry> comparingByValue(IntComparator cmp) {
            Objects.requireNonNull(cmp);
            return (Comparator<Entry> & Serializable)
    ...
    default int replace(int key, int value) {
        int curValue;
        if (((curValue = get(key)) != 0) || containsKey(key)) {
    ...
    default int computeIfAbsent(int key, ToIntIntFunction mappingFunction) {
        ...
        if(!containsKey(key)) {
            int newValue;
            if ((newValue = mappingFunction.applyAsInt(key)) != DEFAULT_INT_VALUE) {
                put(key, newValue);
                return newValue;
            }
        }
    ...
    default int computeIfPresent(int key, IntToIntBiFunction remappingFunction) {
        ...
        if(containsKey(key)) {
            int newValue = remappingFunction.applyAsInt(key, get(key));
    ...
    default int compute(int key, IntToIntBiFunction remappingFunction) {
        ...
        if (newValue == DEFAULT_INT_VALUE) {
            // delete mapping
            if (oldValue != DEFAULT_INT_VALUE || containsKey(key)) {
                // something to remove
                remove(key);
                return DEFAULT_INT_VALUE;
            } else {
     ...
     * @since 2.0
     */
    default int merge(int key, int value, IntToIntBiFunction remappingFunction) {
        Objects.requireNonNull(remappingFunction);
        int oldValue = get(key);
        int newValue = (oldValue == DEFAULT_INT_VALUE) ? value : remappingFunction.applyAsInt(oldValue, value);
        if (newValue == DEFAULT_INT_VALUE) {
            remove(key);
        } else {
            put(key, newValue);
     ...
     * @since 2.0
     */
    static  Entry entry(int k, int v) {
        return new IntKeyIntValue(k, v);
    }
}
