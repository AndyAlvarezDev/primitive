		...
		implements IntSet, Cloneable, java.io.Serializable
{
	...
	private transient HashIntMap<Object> map;
	...
	public HashIntSet(IntCollection c) {
		map = new HashIntMap<>(Math.max((int) (c.size()/.75f) + 1, 16));
		addAll(c);
	}
	...
	public HashIntSet(int initialCapacity, float loadFactor) {
		map = new HashIntMap<>(initialCapacity, loadFactor);
	}
	...
	public HashIntSet(int initialCapacity) {
		map = new HashIntMap<>(initialCapacity);
	}
	...
	HashIntSet(int initialCapacity, float loadFactor, boolean dummy) {
		map = new LinkedHashIntMap<>(initialCapacity, loadFactor);
	}
	...
	public Object clone() {
		try {
			...
			newSet.map = (HashIntMap<Object>) map.clone();
			return newSet;
	...
	private void readObject(java.io.ObjectInputStream s)
		...
		map = (this instanceof LinkedHashIntSet ?
				new LinkedHashIntMap<>(capacity, loadFactor) :
				new HashIntMap<>(capacity, loadFactor));
	...
	public Spliterator.OfInt spliterator() {
		return new HashIntMap.KeySpliterator<>(map, 0, -1, 0, 0);
	}
}
