 ...
 * override the {@link #set(int, int) set(int, E)} method (which otherwise
 * throws an {@code UnsupportedOperationException}).  If the list is
 * variable-size the programmer must additionally override the
 * {@link #add(int, int) add(int, E)} and {@link #remove(int)} methods.
 *
 ...
 * methods:
 * {@link #get(int)},
 * {@link #set(int, int) set(int, E)},
 * {@link #add(int, int) add(int, E)} and
 * {@link #remove(int)}.
 *
...
public abstract class AbstractIntList extends AbstractIntCollection implements IntList {
     ...
     * {@link #add(int, int) add(int, E)} is overridden.
     *
     ...
     */
    static final class RandomAccessSpliterator implements Spliterator.OfInt {
        ...
        RandomAccessSpliterator(IntList list) {
        ...
        private static int get(IntList list, int i) {
            try {
                return list.get(i);
            } catch (IndexOutOfBoundsException ex) {
                throw new ConcurrentModificationException();
    ...
    private static class SubIntList extends AbstractIntList {
        private final AbstractIntList root;
        private final SubIntList parent;
         ...
         */
        public SubIntList(AbstractIntList root, int fromIndex, int toIndex) {
            this.root = root;
            this.parent = null;
         ...
         */
        protected SubIntList(SubIntList parent, int fromIndex, int toIndex) {
        ...
        public int removeFromIndex(int index) {
            Objects.checkIndex(index, size);
            checkForComodification();
            int result = root.removeFromIndex(offset + index);
            updateSizeAndModCount(-1);
            return result;
        }
    ...
    private static class RandomAccessSubIntList
            extends SubIntList implements RandomAccess {

        /**
         * Constructs a sublist of an arbitrary AbstractList, which is
         * not a RandomAccessSubList itself.
         */
        RandomAccessSubIntList(AbstractIntList root,
         ...
         */
        RandomAccessSubIntList(RandomAccessSubIntList parent,
                            int fromIndex, int toIndex) {
