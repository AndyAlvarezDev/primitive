		...
		implements LongSet, Cloneable, java.io.Serializable
{
	...
	private transient HashLongMap<Object> map;
	...
	public HashLongSet(LongCollection c) {
		map = new HashLongMap<>(Math.max((int) (c.size()/.75f) + 1, 16));
		addAll(c);
	}
	...
	public HashLongSet(int initialCapacity, float loadFactor) {
		map = new HashLongMap<>(initialCapacity, loadFactor);
	}
	...
	public HashLongSet(int initialCapacity) {
		map = new HashLongMap<>(initialCapacity);
	}
	 ...
	 *             than zero, or if the load factor is nonpositive
	 */
	HashLongSet(int initialCapacity, float loadFactor, boolean dummy) {
		map = new LinkedHashLongMap<>(initialCapacity, loadFactor);
	}
	...
	public Object clone() {
		try {
			...
			newSet.map = (HashLongMap<Object>) map.clone();
			return newSet;
	...
	private void readObject(java.io.ObjectInputStream s)
		...
		SharedSecrets.getJavaObjectInputStreamAccess()
				.checkArray(s, IntMap.Entry[].class, HashIntMap.tableSizeFor(capacity));

		// Create backing HashMap
		map = (this instanceof LinkedHashLongSet ?
				new LinkedHashLongMap<>(capacity, loadFactor) :
				new HashLongMap<>(capacity, loadFactor));

		// Read in all elements in the proper order.
		for (int i=0; i<size; i++) {
			map.put(s.readInt(), PRESENT);
		}
	}
	...
	public Spliterator.OfLong spliterator() {
		return new HashLongMap.KeySpliterator<>(map, 0, -1, 0, 0);
	}
}
